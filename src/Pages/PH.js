...
function PH() {
  ...
  const getTemp = () => {
    setInterval(() =>{
      Axios.post("http://localhost:3001/getpH",{plantname: plantname},{ withCredentials: true }).then((response) => {
        setsensorread_Temp(response.data[0].pH);
      });
    },1000);
  };
  const getRange = () => {
  ...
  const getControllerStatus = () => {
    Axios.post("http://localhost:3001/getControllerpH",{plantname: plantname},{ withCredentials: true }).then((response) => {
      setChecked1(response.data[0].phcontrolstrategy);
      setChecked2(response.data[0].phcontrolstatus);
    });
  }
  const pushControllerStatus = () => {
    if (checked1 == true) {
      Axios.post("http://localhost:3001/pushControllerpH",{ plantname: plantname, phcontrolstrategy: checked1 },{ withCredentials: true })
    }
    else {
      Axios.post("http://localhost:3001/pushControllerpH",{ plantname: plantname, phcontrolstrategy: checked1},{ withCredentials: true })
      Axios.post("http://localhost:3001/manualpushControllerpH",{ plantname: plantname, phcontrolstatus: checked2},{ withCredentials: true })
    }
  }
  const WrapperFn = () => {
    getTemp();
    getRange();
    setLowertemp(Range[0].lowerpH);
    setHighertemp(Range[0].higherpH);
    getControllerStatus();
  };
      ...
      <Paper elevation={0} style={paperStyle}>
        ...
        <Paper elevation={6} style={paperStyle2}>
          ...
          <Grid container spacing={5} direction="row" justifyContent="center" alignItems="center">
            <Grid className="clight" item xs={2}>Pump</Grid>
